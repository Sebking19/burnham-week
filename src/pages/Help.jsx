...
export default function Help() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="font-display text-3xl text-[#1B2A5B]">Help</h1>
        <p className="text-lg text-[#141B34]/70 mt-1">Who to call if you need a hand with the app or the week</p>
      </div>

      {contacts === null ? (
        <p className="text-lg text-[#141B34]/70">Loading contacts...</p>
      ) : contacts.length === 0 ? (
        ...
        contacts.map((c) => (
          <div key={c.id} className="bg-white border-2 border-dotted border-[#141B34]/40 rounded-2xl p-5 flex items-center gap-4">
            <div className="flex-1">
              <p className="text-xl font-bold text-[#1B2A5B]">{c.name}</p>
              {c.role && <p className="text-lg text-[#141B34]/70">{c.role}</p>}
      ...
      <div className="bg-white border-2 border-dotted border-[#141B34]/40 rounded-2xl p-6">
        <h2 className="font-display text-2xl text-[#1B2A5B]">On the water</h2>
        <p className="text-lg text-[#141B34]/85 mt-2">
